    ...
    >
      <template slot="items" slot-scope="props">
        <td>
            <v-avatar size="32px">
            <img
                :src="`${base}/nurses/${props.item.image}`"
                alt="profile"
                class=""
            >
        </v-avatar>
...
import {apiDomain,IMAGE_URL} from '@/utils/constants'
  export default {
    ...
    mounted(){
        this.loading = true
        this.nurses.url =  apiDomain +"nurse"
        this.$store.dispatch('getReqData',this.nurses).then((res)=>{
    ...
    methods:{
       ...
       editItem(id){
            var item = this.nurses.find(item => item.id === id);
            this.dialog = true
        ...
        save(){
           //if(!this.editedIndex.image)
            ...
            this.$validator.validateAll().then(()=>{
                if(!this.errors.any()){
                    this.editedItem.url = apiDomain + "nurse/add"
                    ...
                    this.$store.dispatch('postReqData',this.editedItem).then((res)=>{
                        ...
                        if(this.editedItem.id){
                            var index = this.nurses.findIndex(item => item.id === this.editedItem.id)
                            this.nurses[index] = {...this.editedItem} 
                        }else{
                        this.nurses.push(this.editedItem)                        
                        }
                        this.dialog = false
                        this.reset()
