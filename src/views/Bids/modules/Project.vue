    ...
    <v-card>
        ...
        <template v-slot:items="props">
            <tr :class="{invited:props.item.status == 1,shortlist:props.item.status == 3}">
                ...
                <td>
                    <v-chip @click="viewMedia(`vat/${props.item.vat}`)" label>
                        <v-icon left>all_out</v-icon>Vat
                    </v-chip>
                </td>
                <td>
                    <v-chip @click="viewMedia(`tax/${props.item.tax}`)" label>
                        <v-icon left>monetization_on</v-icon>Tax
                    </v-chip>
                </td>
                <td>
                    <v-chip @click="viewMedia(`business/${props.item.business}`)" label>
                        <v-icon left>business</v-icon>Business
                    </v-chip>
                </td>
                <td>
                    <v-chip label>
                        <a :href="`${FILE_URL}/${props.item.cv}`"  :download="`${props.item.company_name}-cv`">
                        <v-icon left>attach_file</v-icon>CV                            
                        </a>
                    </v-chip>
                </td>
                ...
                <td style="width:20%">
                    ...
                    <v-tooltip left>
                        <template v-slot:activator="{ on }">
                            <v-btn   @click="viewGrant(props.item.id,props.item.company_name)" fab small color="#2c3e50" dark v-on="on">
                                <v-icon>gavel</v-icon>
                            </v-btn>
                        </template>
                        <span>Grant</span>
                    ...
                    <v-tooltip left>
                        <template v-slot:activator="{ on }">
                            <v-btn   @click="viewGrant(props.item.id)" fab small color="warning" dark v-on="on">
        ...
        <template v-slot:items="props">
            <tr :class="{invited:props.item.status == 1}">
                ...
                <td style="width:20%">
                    ...
                    <v-tooltip left>
                        <template v-slot:activator="{ on }">
                            <v-btn   @click="viewGrant(props.item.id)" fab small color="#C1A724" dark v-on="on">
                                <v-icon>verified_user</v-icon>
                            </v-btn>
                        </template>
                        <span>verified_user</span>
...
import {apiDomain,IMAGE_URL,FILE_URL} from '@/utils/constants'
    export default {
        ...
        methods:{
            generateReport(){
                this.$router.push({name:'Bid Report',params:this.$route.params.id})
            },
            getBids(){
